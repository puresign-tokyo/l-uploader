...
<v-card
  class="d-flex my-8"
  elevation="4"
  style="border-left: 8px solid #FF75DD; align-items: stretch; min-height: 170px;"
  replay
>
  <!-- 画像 -->
  ...
  <v-col class="pa-2 d-flex justify-space-between align-start" style="flex: 1 1 auto;">
    <v-card-text class="pa-0" elevation="0">
      <v-row no-gutters>
        ...
        <v-col cols="12" class="d-flex align-center">
          ...
          <strong>東方妖々夢 ～ Perfect Cherry Blossom.</strong>
        </v-col>
        ...
        <v-col cols="4" md="3" class="d-flex align-center mt-1">
          ...
          <!-- 月日の情報しか入っていない.年の情報も仮で入っているが使ってはいけない -->
          {{ new Date(replay.replay_meta.timestamp).toLocaleString('ja-JP', { month: '2-digit', day: '2-digit'})  }}
        </v-col>
    ...
    <!-- アイコン群（右上） -->
    <div class="d-flex align-center" style="position: absolute; bottom: 8px; right: 12px; gap: 8px;">
      ...
      <v-icon
        ...
        variant="tonal"
        color="error"
        @click="$emit('confirmDelete',replay)"
        title="削除"
      />
      ...
      <v-icon
        ...
        variant="tonal"
        @click="$emit('confirmShare',replay)"
        title="シェア"
      />

      <a :href="`${useRuntimeConfig().public.backend_url}/replays/${replay.replay_id}/file`" target="_blank" rel="noopener" style="text-decoration: none; color: inherit;">
        <v-icon
