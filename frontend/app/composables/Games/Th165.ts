...
function convertEnemyPair(enemy1: {ja: string, en: string}, enemy2: {ja: string, en: string}){
  return {ja: `${enemy1.ja} & ${enemy2.ja}`, en: `${enemy1.en} & ${enemy2.en}`}
}
...
const spellCardMap: Record<string, Record<string, Spell>>={
  ...
  17: { //悪夢火曜
    ...
    2: {
      ...
      boss: convertEnemyPair(convertEnemy('Tenshi'), convertEnemy('Sinmyoumaru'))
    },
    3: {
      spellCard: {
        ja: '永緋符「墜落する壺中の有頂天」'
    ...
    4: {
      ...
      boss: convertEnemyPair(convertEnemy('Kaguya'), convertEnemy('Sinmyoumaru'))
    },
    5: {
      spellCard: {
        ja: '永輝符「大きくなる壺」'
      },
      boss: convertEnemyPair(convertEnemy('Eirin'), convertEnemy('Sinmyoumaru'))
    },
    6: {
      spellCard: {
        ja: '緋夜符「蓬莱の弾の要石」'
...
export function Th165Table(replay: Th165Replay){
  ...
  return{
    ...
    difficulty: {label: convertLevel(String(replay.replay_meta.level)).ja+' - '+String(replay.replay_meta.scene), color: difficultyColor},
    shot_type: null,
