...
class GameRegistry:
    ...
    def identify_with_replayfile(cls, rep_raw: bytes) -> GameEntry:
        ...
        raise ValueError(
            f"Unsupported replay file format. File magic value: {rep_raw[:4].hex()}. For th13/th14, metadata check failed to identify the game."
        )

    @classmethod
    def supported_game_ids(cls):
        return list(GameRegistry._games.keys())
